...
def write_calibration_parameters(mx, cx, my, cy):
    """
    Writes the calibration parameters (mx, cx, my, cy) to an Excel file named
    'Project_parameters_file.xlsx' in the 'calibration_parameters' sheet.

    If the sheet already exists, it will be refreshed with the new data.

    Args:
        mx (float): Scaling factor for x-coordinate.
        cx (float): Offset for x-coordinate.
        my (float): Scaling factor for y-coordinate.
        cy (float): Offset for y-coordinate.
    """
    ...
    print(f"Calibration parameters written to '{file_name}' in the '{sheet_name}' sheet.")
...
def detect_markers_once(cap):
    ...
    while True:
        ...
        if key == ord('e') and marker_set_1_detected:
            ...
            filename = f"E:/RASLAB/PROJECT_RUN_THROUGH/captured_frame_{timestamp}.png"            
...
def main():
    ...
    write_calibration_parameters(mx, cx, my, cy)
    print("\n")
    print(f"mx = {mx}, my = {my}")
    print(f"cx = {cx}, cy = {cy}")
