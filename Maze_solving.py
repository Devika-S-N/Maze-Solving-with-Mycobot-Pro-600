...
# Replace the automatic detection with manual selection
def select_points(image):
    points = []
    def mouse_callback(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            points.append((y, x))  # Note: OpenCV uses (x,y) but we need (row,col)
...
#%%
def main():
    img_new_name = input("Please enter the name of the image file: ")
    # Load the maze image
    img_name = f'E:/RASLAB/PROJECT_RUN_THROUGH/{img_new_name}.png'  # Replace with your maze image
    img = cv2.imread(img_name, cv2.IMREAD_GRAYSCALE)
    maze_image_non_rotated = img
    #img = cv2.flip(maze_image_non_rotated, 0)# mirror through y
    img = cv2.rotate(maze_image_non_rotated, cv2.ROTATE_90_CLOCKWISE)
    #img_non_flipped = cv2.flip(img, 0)# mirror through x
    #img = img_non_flipped
    
    
    
    modified_img = cv2.convertScaleAbs(img, alpha=1.7, beta=-100)    
    ...
    # Use this function instead of detect_entrance_exit
    #print("Please click on the entrance point, then the exit point.")
    #selected_points = select_points(adjusted_maze)

    #if len(selected_points) == 2:
    #    entrance, exit_point = selected_points
    #    print(f"Entrance: {entrance}, Exit: {exit_point}")
    #else:
    ...
    for i in range(len(aligned_points) - 1):
        ...
        cv2.line(solution_img, pt1, pt2, (0, 0, 255), thickness=2)
    
    solution_points = []
    solution_points.append(entrance)
    # Draw the turn points
    for point in aligned_points[1:-1]:  # Exclude start and end
        x, y = point
        solution_points.append(point)
        cv2.circle(solution_img, (y, x), 5, (0, 255, 0), -1)  # Green circles at turns
        print(f"x coordinate: {x}, y coordniate: {y}")
    solution_points.append(exit_point)

    # Draw entrance and exit points
    cv2.circle(solution_img, (entrance[1], entrance[0]), 5, (255, 0, 0), -1)  # Blue circle for entrance
    cv2.circle(solution_img, (exit_point[1], exit_point[0]), 5, (255, 0, 0), -1)  # Blue circle for exit


    aligned_points = list(dict.fromkeys(aligned_points))
    add_pixel_coordinates(file_name,"Pixel_coordinates",solution_points)
    #img_name = "captured_frame_1733336191"
    save_path_instructions(aligned_points[0:], file_append=img_new_name)
